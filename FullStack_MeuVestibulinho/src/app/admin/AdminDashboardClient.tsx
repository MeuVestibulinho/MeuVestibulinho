...
type UserListItem = inferRouterOutputs<AppRouter>["admin"]["listUsers"]["items"][number];
type Props = {
...
export default function AdminDashboardClient({ currentUserId, initialOverview }: Props) {
  ...
  const [uploadFeedback, setUploadFeedback] = React.useState<{ ano: number; message: string; type: "success" | "error" } | null>(null);
  ...
  React.useEffect(() => {
    if (usersQuery.data && page > usersQuery.data.meta.totalPages) {
      setPage(usersQuery.data.meta.totalPages);
    }
  }, [page, usersQuery.data]);

  const simuladosQuery = api.admin.simulados.useQuery(undefined, { staleTime: 60_000 });
  ...
  const handleUpload = React.useCallback(
    async (ano: number, file: File) => {
      ...
      try {
        ...
        setUploadFeedback({ ano, message: "Imagem atualizada com sucesso!", type: "success" });
        await Promise.all([
  ...
  const handleFileChange = React.useCallback(
    (ano: number, event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file.size > 5 * 1024 * 1024) {
        setUploadFeedback({ ano, message: "Selecione uma imagem de até 5MB.", type: "error" });
        event.target.value = "";
        return;
      }
    ...
    <div className="mx-auto max-w-7xl px-4 pb-16 pt-24">
      ...
      <section className="mb-10 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
          <p className="text-xs font-semibold uppercase text-gray-500">Usuários ativos</p>
          <p className="mt-2 text-3xl font-bold text-gray-900">{overview.totalUsers}</p>
          <p className="mt-2 text-xs text-gray-500">{overview.adminCount} administradore{overview.adminCount === 1 ? "" : "s"}.</p>
        </div>
        <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
          <p className="text-xs font-semibold uppercase text-gray-500">Estatísticas registradas</p>
          <p className="mt-2 text-3xl font-bold text-green-600">{overview.statsRegistradas}</p>
          <p className="mt-2 text-xs text-gray-500">Usuários com histórico de simulados.</p>
        </div>
        <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
          <p className="text-xs font-semibold uppercase text-gray-500">E-mails bloqueados</p>
          <p className="mt-2 text-3xl font-bold text-red-600">{overview.blacklistCount}</p>
          <p className="mt-2 text-xs text-gray-500">Proteção contra recriações maliciosas.</p>
        </div>
        <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
          <p className="text-xs font-semibold uppercase text-gray-500">Simulados disponíveis</p>
          <p className="mt-2 text-3xl font-bold text-gray-900">{overview.simuladosDisponiveis}</p>
          <p className="mt-2 text-xs text-gray-500">{overview.simuladosComCapa} com capa personalizada.</p>
        </div>
      </section>

      <section className="mb-12 space-y-4 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Gestão de usuários</h2>
            <p className="text-xs text-gray-500">
              Pesquise por nome, e-mail ou ID para atualizar permissões e revisar estatísticas individuais.
            </p>
          </div>
          <input
            type="search"
                  ...
                  return (
                    <tr key={user.id} className="bg-white">
                      ...
                      <td className="px-4 py-3">
                        ...
                        <p className="mt-1 text-[11px] text-gray-400">Desde {formatDate(user.createdAt)}</p>
                      </td>
                      <td className="px-4 py-3 text-sm text-gray-700">{statsSummary}</td>
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap justify-end gap-2">
                          ...
                          {user.role !== "ADMIN" ? (
                            ...
                            <button
                              ...
                              disabled={isCurrentUser || demoteMutation.isPending}
                              className={clsx(
        ...
        {meta && (
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="text-xs text-gray-500">
              Página {meta.page} de {meta.totalPages} · {meta.total} usuário{meta.total === 1 ? "" : "s"}
            </p>
      ...
      <section className="space-y-4 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Capas dos simulados</h2>
            <p className="text-xs text-gray-500">
              Personalize as imagens exibidas nos cards. Formatos suportados: PNG, JPG, WEBP ou AVIF até 5MB.
            </p>
          </div>
        </div>
        ...
        {simuladosQuery.isLoading ? (
          ...
          <p className="rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-6 text-center text-sm text-gray-500">
            Nenhum simulado disponível. Cadastre questões para liberar esta área.
          </p>
        ) : (
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {simulados.map((item) => {
              const metadata = item.metadata;
              const capa = metadata?.coverImageUrl ?? coverForYear(item.ano) ?? COVER_FALLBACK;
              const feedbackActive = uploadFeedback && uploadFeedback.ano === item.ano;

              return (
                <div key={item.ano} className="flex h-full flex-col overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm">
                  ...
                  <div className="flex flex-1 flex-col gap-4 p-5">
                    <div className="flex items-center justify-between text-xs font-medium text-gray-600">
                      <span className="rounded-full bg-red-50 px-3 py-1 text-red-700">Ano {item.ano}</span>
                      <span className="rounded-full bg-orange-50 px-3 py-1 text-orange-700">
                        {item.questoes} questão{item.questoes === 1 ? "" : "s"}
                      </span>
                    </div>
                    <p className="text-sm text-gray-600">
                      {metadata?.descricao ?? "Atualize a capa para destacar este simulado na vitrine principal."}
                    </p>
                    <div className="mt-auto space-y-2">
                      <label className="flex cursor-pointer flex-col items-start gap-2 text-xs font-semibold text-gray-700">
                        ...
                        <input
                          ...
                          className="hidden"
                          onChange={(event) => handleFileChange(item.ano, event)}
                        />
                      </label>
                      {feedbackActive && (
                        <p
                      ...
                      <p className="text-[11px] text-gray-400">
                        {metadata?.updatedAt ? `Última atualização: ${formatDate(metadata.updatedAt)}` : "Capa padrão"}
                      </p>
                    </div>
                  </div>
        ...
        >
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="text-xl font-semibold text-gray-900">Estatísticas de {selectedUser.name ?? selectedUser.email ?? "usuário"}</h3>
            ...
            {userStatsQuery.isLoading ? (
              <p className="mt-6 text-sm text-gray-500">Carregando estatísticas...</p>
            ) : userStatsQuery.data ? (
              <div className="mt-6 space-y-4">
                <div className="grid gap-4 md:grid-cols-3">
                  <div className="rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3">
                    <p className="text-xs font-semibold uppercase text-gray-500">Acertos</p>
                    <p className="text-xl font-bold text-green-600">{userStatsQuery.data.totalAcertos}</p>
                  </div>
                  <div className="rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3">
                    <p className="text-xs font-semibold uppercase text-gray-500">Erros</p>
                    <p className="text-xl font-bold text-red-600">{userStatsQuery.data.totalErros}</p>
                  </div>
                  <div className="rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3">
                    <p className="text-xs font-semibold uppercase text-gray-500">Simulados</p>
                    <p className="text-xl font-bold text-gray-800">{userStatsQuery.data.simuladosConcluidos}</p>
                  </div>
                </div>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {userStatsQuery.data.conteudosDetalhados.length === 0 ? (
                        ...
                        userStatsQuery.data.conteudosDetalhados.map((item) => (
                          <tr key={`${item.conteudo}-${item.subconteudo}-admin`}
                            className="bg-white"
                          >
              ...
              <p className="mt-6 text-sm text-gray-500">Sem estatísticas registradas para este usuário.</p>
            )}
          </div>
        </div>
      )}
