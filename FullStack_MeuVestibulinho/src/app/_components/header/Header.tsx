...
function initials(label?: string): string {
  ...
  // remove parte do email, se houver
  const at = base.indexOf("@");
  const clean = at >= 0 ? base.slice(0, at) : base;

  const parts = clean.split(/\s+/).filter(Boolean); // string[]

  // aceita string opcional; nunca acessa índice inseguro
  const pick = (s?: string): string => (s?.charAt(0)?.toUpperCase() ?? "");

  if (parts.length >= 2) {
    return pick(parts[0]) + pick(parts[1]); // ok: pick aceita opcional
  }
...
export default async function Header() {
  ...
  const RightSlot = (
    <>
      {!user ? (
        // Botão Entrar — overlay gradiente + underline animado (igual aos itens)
        <a
          href="/auth/signin"
          aria-label="Entrar"
        ...
        <div className="flex items-center gap-3">
          {/* Pill com avatar + nome e realce no hover */}
          ...
          {/* Botão Sair — mesmo padrão de hover dos itens */}
          <form
            action={async () => {
              "use server";
