...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ asChild, variant = "primary", className, children, ...props }, ref) => {
    ...
    const variants = {
      primary: "bg-red-600 text-white shadow-lg hover:shadow-2xl",
      secondary: "border-2 border-gray-300 text-gray-800 hover:border-red-500",
    };
    ...
    return (
      <motion.div
        onHoverStart={() =>
          controls.start({ scale: [1, 1.4, 1.2, 1.5], rotate: [0, 10, -10, 0] })
        }
        onHoverEnd={() =>
          controls.start({
            scale: 1,
