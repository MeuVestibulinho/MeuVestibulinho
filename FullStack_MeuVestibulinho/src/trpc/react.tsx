...
const getQueryClient = () => { // Função que retorna a instância do QueryClient
...
export function TRPCReactProvider(props: { children: React.ReactNode }) {
  ...
  const [trpcClient] = useState(() => // Cria o cliente TRPC apenas uma vez usando useState
    api.createClient({ // Cria o cliente TRPC
      links: [ // Define os links usados pelo cliente TRPC para comunicação com o servidor
        loggerLink({ // Link que loga as requisições e respostas do TRPC para facilitar o debug
          enabled: (op) => // Função que determina quando o logging deve estar habilitado
        ...
        httpBatchStreamLink({ // Link que permite fazer requisições HTTP em lote e com streaming
          ...
          headers: () => { // Função que retorna os headers a serem enviados em cada requisição
  ...
  return (
    <QueryClientProvider client={queryClient}> {/* Provê o contexto do React Query para os componentes filhos */}
      <api.Provider client={trpcClient} queryClient={queryClient}> {/* Provê o contexto do TRPC para os componentes filhos */}
