...
export const createQueryClient = () =>
  new QueryClient({ // Cria uma nova instância do QueryClient
    defaultOptions: { // Define opções padrão para o QueryClient
      queries: { // Configurações específicas para consultas (queries)
      ...
      dehydrate: { // Configurações para desidratação (dehydrate) e hidratação (hydrate) de dados, úteis para SSR
        serializeData: SuperJSON.serialize, // Usa SuperJSON para serializar os dados antes de enviá-los ao cliente
        shouldDehydrateQuery: (query) => // Personaliza a lógica para decidir se uma consulta deve ser desidratada
          defaultShouldDehydrateQuery(query) || 
          query.state.status === "pending", // Também desidrata consultas que estão no estado "pending" (pendente)
      },
      hydrate: { // Configurações para hidratação de dados no cliente
