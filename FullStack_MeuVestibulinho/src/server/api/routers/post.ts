...
export const postRouter = createTRPCRouter({
  hello: publicProcedure // Rota pública que retorna uma saudação personalizada
    ...
    .query(({ input }) => {
      // Define a função que será executada quando essa rota for chamada, recebendo o input validado
      return {
        // Retorna um objeto com a saudação personalizada
        greeting: `Hello ${input.text}`, // Usa o valor de input.text para criar a saudação
      };
    }),

  getSecretMessage: protectedProcedure.query(() => {
    // Rota protegida que retorna uma mensagem secreta, acessível apenas para usuários autenticados
